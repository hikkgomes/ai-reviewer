...
from dissect_checks.test_integrity.static_analysis import analyse_static  # noqa: E402
...
def validate(manifest: Any, calibration: Any | None = None) -> list[str]:
    ...
    for rule_id in enabled:
        ...
        if not isinstance(record.get("positive_cases"), list) or len(record.get("positive_cases", [])) < 3:
            errors.append(f"rule {rule_id} needs at least three positive cases")
        elif all(isinstance(item, str) for item in record["positive_cases"]) and len(set(record["positive_cases"])) != len(record["positive_cases"]):
            errors.append(f"rule {rule_id} positive cases must be unique")
        if not isinstance(record.get("negative_cases"), list) or len(record.get("negative_cases", [])) < 8:
            errors.append(f"rule {rule_id} needs at least eight negative cases")
        elif all(isinstance(item, str) for item in record["negative_cases"]) and len(set(record["negative_cases"])) != len(record["negative_cases"]):
...
def _static_meta_mutation(root: Path) -> list[str]:
    ...
    errors: list[str] = []
    for rule_id in (f"GOV-TESTS-{index:03d}" for index in range(1, 7)):
        base = dict(head)
        ...
        if not any(item.get("source") == rule_id for item in result.candidates):
            errors.append(f"{rule_id} acceptance corpus produced no candidate")
        ...
        if any(item.get("source") == rule_id for item in disabled.candidates):
            errors.append(f"disabling {rule_id} did not remove its candidate")
    return errors
...
def validate_rule_ownership(root: Path) -> list[str]:
    ...
    for rule_id, expected_owner in sorted(discovered.items()):
        if owner_for(rule_id) != expected_owner:
...
def _complexity_meta_mutation(root: Path) -> list[str]:
    ...
    try:
        green = replace(result, candidates=())
        if not green.candidates:
            raise AssertionError("complexity acceptance depends on measured candidates")
    except AssertionError:
        return []
    return ["complexity green-result meta-mutation was not rejected"]


def meta_mutation_errors(root: Path, *, require_toolchains: bool = False) -> list[str]:
    errors = _static_meta_mutation(root)
    errors.extend(_complexity_meta_mutation(root))
    for language, values in CASES.items():
        for rule_id in values["rules"]:
            try:
                run_acceptance(languages=(language,), require_toolchains=require_toolchains, disabled_rule=rule_id)
